                ...
                if (!context.Curso.Any())
                {
                    ...
                    if (!context.Requisito.Any())
                    {
                        ...
                        if (!context.Documento.Any())
                        {
                            ...
                            {
                                await ServicoAtividadeEstagio.CadastrarAtividadeParaEntidade(doc, 1);
                            }
                            foreach(Documento doc in DocumentosADM)
                            {
                                await ServicoAtividadeEstagio.CadastrarAtividadeParaEntidade(doc, 2);
                            }
                            foreach(Documento doc in DocumentosCC)
                            {
                                await ServicoAtividadeEstagio.CadastrarAtividadeParaEntidade(doc, 3);
                            }
                            foreach(Documento doc in DocumentosLET)
                            {
                                await ServicoAtividadeEstagio.CadastrarAtividadeParaEntidade(doc, 4);
                            }
                            foreach(Documento doc in DocumentosPED)
                            {
                                await ServicoAtividadeEstagio.CadastrarAtividadeParaEntidade(doc, 5);
                            }
                            #endregion
